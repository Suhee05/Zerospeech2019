...
def DiscretizedMixtureLogisticLoss(outputs, targets, config, lengths=None, mask=None, max_len=None):
	if lengths is None and mask is None:
		raise RuntimeError('Please provide either lengths or mask')
	...
	losses = discretized_mix_logistic_loss(
		outputs, targets, num_classes=config.quantize_channels,
		log_scale_min=config.log_scale_min, reduce=False)
...
def GaussianMaximumLikelihoodEstimation(outputs, targets, config, lengths=None, mask=None, max_len=None):
	if lengths is None and mask is None:
		raise RuntimeError('Please provide either lengths or mask')
	...
	losses = gaussian_maximum_likelihood_estimation_loss(
		outputs, targets, log_scale_min_gauss=config.log_scale_min_gauss,
		num_classes=config.quantize_channels, use_cdf=config.cdf_loss, reduce=False)

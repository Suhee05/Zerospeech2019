...
def mfcc(sample, hparams):
    mfcc = _mfcc(sample, hparams)
    d = _delta(mfcc)
    a = _delta(d)
    feature = np.concatenate((mfcc,d,a)).T
    return feature

def _mfcc(sample, hparams):
    S = melspectrogram(sample, hparams)
    dct_type=2
    norm='ortho'
    return dct(S, axis=0, type=dct_type, norm=norm)[:hparams.n_mfcc]
...
def _stft(y, hparams):
	return librosa.stft(y=y, n_fft=hparams.n_fft, hop_length=get_hop_size(hparams), win_length=hparams.win_size, pad_mode='constant', center=hparams.sample_padding)

...
from utils.util import is_mulaw_quantize, is_scalar_input
...
class Feeder:
	...
	def __init__(self, coordinator, metadata_filename, base_dir, hparams):
		...
		with tf.device('/cpu:0'):
			...
			if self.local_condition:
				self._placeholders.append(tf.placeholder(tf.float32, shape=(None, None, hparams.num_mfccs), name='local_condition_features'))
	...
	def _get_test_groups(self):
		...
		if self.local_condition:
			local_condition_features = np.load(os.path.join(self._base_dir, mfcc_file))
		else:
	...
	def _get_next_example(self):
		...
		if self.local_condition:
			if self._hparams.pre_train_mode:
				local_condition_features = None
			local_condition_features = np.load(os.path.join(self._base_dir, mfcc_file))
